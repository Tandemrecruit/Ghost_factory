...
Details:
- Event types: `page_view`, `cta_click`, `conversion` â€” minimal set for measuring page performance.
- No PII, no cookies, no localStorage â€” aggregate-friendly anonymous data only.
...
- Python modules in `automation/**` and other core codepaths should favor correctness and observability over â€œclever minimalism.â€
- Non-trivial flows must include structured logging (info/warn/error) and clear failure modes that surface actionable errors.
Rationale:
- Ghost Factory emits paid client deliverables; oversimplified code can hide edge cases and make failures harder to debug than well-structured â€œcomplexâ€ code.
Impact:
