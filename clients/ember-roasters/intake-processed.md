# Client Overview
- Business Name: Ember Roasters
- Tagline: "Coffee Worth Waking Up For"
- Location: Portland, Oregon
- Industry / Niche: Coffee Roastery; Direct-to-Consumer
- Current Website (if any): Not provided (Instagram: @emberroasterspdx)
- Primary Offer: Monthly coffee subscription boxes
- Secondary Offer(s): One-time bag purchases
- Ideal Customer: 28â€“45-year-olds who value quality coffee, have a nice home brewing setup (pour-over or espresso), and are willing to pay more for quality; not the Folgers crowd
- Brand Voice (3â€“5 words): warm, artisan, unpretentious, quality-obsessed
- Main Goal of This Page: Sell subscriptions (primary); drive one-time purchases (secondary)

# Core Messaging
- Core Promise (1â€“2 sentences):
  Ultra-fresh, small-batch coffee delivered to your door each month, roasted weekly and sourced directly from farmers. Every bag shows a roast date, and subscriptions include free shipping.
- Top 3 Benefits:
  - Fresh weekly roasts
  - Direct-from-farm sourcing (Ethiopia, Colombia, Guatemala)
  - Free shipping on subscriptions
- Key Objections & Reassurances:
  - Objection 1: "subscription coffee is too expensive"
    - Reassurance: About $0.50 per cup, cheaper than Starbucks
  - Objection 2: "what if i dont like it"
    - Reassurance: Satisfaction guarantee; first bag free replacement
  - Objection 3: "i dont want to commit"
    - Reassurance: Pause or cancel anytime; no contracts
- Urgency / Time Sensitivity: Not time-sensitive; ongoing subscriptions

# Page Requirements
- Primary CTA (text + behavior): Start Your Subscription
- Primary CTA Target (phone / URL / email): URL: TBD
- Secondary CTA (if any): Not provided
- Required Sections:
  - Hero section
  - Features / Benefits
  - Testimonials (optional)
  - Pricing section (3 tiers)
  - FAQ
- Optional / Nice-to-Have Sections:
  - Press / Media mentions (Portland Monthly; Willamette Week)
  - About the farmers / Sourcing story
- Must-Avoid Topics / Phrases:
  - Avoid elitist or pretentious language
  - Avoid false scarcity or hype
  - Avoid competitor-bashing

# Assets & Constraints
- Logo / Brand Assets: Not provided
- Existing Brand Colors: Deep burgundy/maroon, cream/off-white, burnt orange
- Existing Copy/Pages to Reuse: Not provided
- Social Proof / Testimonials: Not provided
- Competitors / Inspiration URLs: Not provided
- Service Area / Locations: United States
- Pricing Strategy (starting price / range / â€œcontact usâ€): Starting price $24/mo; tiers: Explorer $24/mo; Enthusiast $42/mo; Obsessed $72/mo; Free shipping on subscriptions
- Legal / Compliance Notes: Mention the satisfaction guarantee

# Project Logistics
- Project Urgency / Timeline: 2 weeks
- Notes about scheduling, hours, availability: Not provided

# Notes from Client
- Special instructions: Include satisfaction guarantee
- Anything unclear / missing: Shopify checkout URL; press quotes; testimonials; website URL; shipping details; exclusive micro-lots; pricing page copy; social proof
